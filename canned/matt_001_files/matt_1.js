...
(function() {
  var M, Viz, individuals_max, individuals_min, least, most, range,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
  ...
  Viz = (function() {
    function Viz() {
      this.mainInt = bind(this.mainInt, this);
      this.documentReady = bind(this.documentReady, this);
      $(document).ready(this.documentReady);
    }

    Viz.prototype.documentReady = function() {
      this.individualC = 0;
      this.osc = M.node({
      ...
      this.verb = M.node({
        node_type: 'Convolver',
        buffer_source_file: 'matt_001_files/st-andrews-church-ortf-shaped.wav'
      });
      setTimeout(this.mainInt, 10);
      return this.osc.start().chain(this.delay).chain(this.verb);
    };

    Viz.prototype.mainInt = function() {
      this.individualC += 1;
      if (this.individualC === individuals_perc.length) {
        this.individualC = 0;
      }
      this.osc.param({
        frequency: 250 + 500 * individuals_perc[this.individualC],
        at: M.context.currentTime + 0.01,
        from_now: true,
        ramp: 'expo'
      });
      $("#freq").text(Math.round(250 + 500 * individuals_perc[this.individualC]));
      return setTimeout(this.mainInt, 10);
    };
